...
def process(oriImg, model):
    ...
    cands_out = np.zeros((len(subset), 19, 2))

    for cand in range(len(subset)):
        positive_indicies = [(limb_idx, int(cand_idx)) for limb_idx, cand_idx
                             in enumerate(subset[cand][:-2]) if cand_idx >= 0]
        cand_index = [c for _, c in positive_indicies]
        limb_index = [l for l, _ in positive_indicies]

        cand_pos = candidate[cand_index][:, :2]

        cands_out[cand][limb_index] = cand_pos
